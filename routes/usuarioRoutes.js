...
router.post('/', verificarToken(['Administrador', 'RH']), async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO usuarios (nome, email, senha, role) VALUES (?, ?, ?, ?)', [nome, email, senha, role]);
...
router.put('/:id', verificarToken(['Administrador', 'RH']), async (req, res) => {
    ...
    try {
        if (senha && senha.trim() !== "") {
            await db.query('UPDATE usuarios SET nome = ?, email = ?, role = ?, senha = ? WHERE id = ?', [nome, email, role, senha, req.params.id]);
        } else {
            await db.query('UPDATE usuarios SET nome = ?, email = ?, role = ? WHERE id = ?', [nome, email, role, req.params.id]);
        }
        res.json({ mensagem: "Usuário atualizado!" });
    } catch (error) { 
        if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ erro: "Este e-mail já está em uso por outro usuário." });
